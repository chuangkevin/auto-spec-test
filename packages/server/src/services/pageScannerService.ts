...
### 清空/保留邏輯必須精確
- 如果規格書有「清除對照表」或類似的條件交互規則，**必須完全遵守**，不能自己推測哪些條件會被清空
- 例：規格書說「切換現況時，保留地區/捷運站」→ 就**不能**期望地區被清空回預設值
- 例：規格書說「清除條件保留現況、地區」→ 就**不能**期望地區或現況改變
- 每個涉及「清空」的測試案例，預期結果中**必須明確標註**哪些條件保留、哪些清空，且必須符合 spec

### 驗證點的可靠性
- **不要用截圖來驗證「列表有顯示」**— 頁面可能需要向下捲動才看到列表，截圖只拍首屏
...
2. 如果截圖顯示的行為與領域知識矛盾，以截圖為準，在 actualResult 中標注「與領域知識描述不符，但截圖顯示...」
3. 領域知識中的 URL 格式、業務規則只是提示，實際行為可能因版本更新而不同
